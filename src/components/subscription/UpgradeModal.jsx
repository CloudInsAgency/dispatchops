...
const UpgradeModal = ({ isOpen, onClose, currentPlan, reason }) => {
  ...
  const handleUpgrade = async () => {
    ...
    try {
      ...
      // Create checkout session - use different URL for dev vs production
        ...
        <div className="p-6">
          ...
          <div className="mb-6 p-4 bg-gray-50 rounded-lg">
            ...
            <p className="text-sm text-gray-600">
              Up to {PLANS[currentPlan].techLimit} technicians
            </p>
          </div>
          ...
          <div className="border-2 border-primary-600 rounded-xl p-6 mb-6 bg-primary-50">
            ...
            <button
              ...
              disabled={loading}
              className="w-full bg-primary-600 text-white py-4 rounded-lg font-semibold text-lg hover:bg-primary-700 transition disabled:opacity-50 shadow-lg"
            >
              {loading ? 'Processing...' : `Upgrade to ${upgradePlan.name}`}
            </button>
          </div>

          {/* Fine Print */}
          <div className="text-center text-sm text-gray-500">
            <p>Your card will be charged ${upgradePlan.price}/month.</p>
            <p className="mt-1">You can cancel or downgrade anytime.</p>
          </div>
        </div>
      </div>
    </div>
