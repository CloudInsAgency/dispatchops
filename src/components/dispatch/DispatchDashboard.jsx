...
const DispatchDashboard = () => {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
                ...
                {filteredJobs.length === 0 ? (
                  ...
                  filteredJobs.map(job => (
                    <tr key={job.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="text-sm text-gray-900">{new Date(job.scheduledDateTime).toLocaleDateString()}</div>
                        <div className="text-sm text-gray-500">{new Date(job.scheduledDateTime).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        {job.assignedToName ? (
                          ...
                          <button onClick={() => handleQuickAssign(job)} className="text-sm text-blue-600 hover:text-blue-700">
                            Assign Tech
                          </button>
                        )}
                      </td>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                        <div className="flex items-center justify-end gap-2">
                          <button onClick={() => { setSelectedJob(job); setShowEditModal(true); }} className="text-blue-600 hover:text-blue-900">Edit</button>
                          <button onClick={() => handleQuickAssign(job)} className="text-green-600 hover:text-green-900">Assign</button>
      ...
      {showCreateModal && (
        <CreateJobModal companyId={userProfile?.companyId} techs={techs} onClose={() => setShowCreateModal(false)} />
      )}
      {showEditModal && selectedJob && (
        <EditJobModal job={selectedJob} techs={techs} onClose={() => { setShowEditModal(false); setSelectedJob(null); }} />
      )}
      {showAssignModal && selectedJob && (
        <AssignTechModal job={selectedJob} techs={techs} onClose={() => { setShowAssignModal(false); setSelectedJob(null); }} />
      )}
      <UpgradeModal
